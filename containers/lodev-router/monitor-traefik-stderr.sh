...
# and logs only ERR (error) messages to /tmp/lodev-traefik-errors.txt.
# WRN (warning) and debug lines are not written there so they do not appear
# as "router configuration problems" in lodev start / lodev list.
...
error_file="/tmp/traefik_errors.txt"
