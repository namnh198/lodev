...
# using nodejs_version from .lodev/config.yaml in lodev-webserver
...
if [ ! -d "/mnt/lodev_default/n_prefix/${HOSTNAME}" ]; then
  echo "This script requires the directory /mnt/lodev_default/n_prefix/${HOSTNAME}" >&2
  exit 4
fi
...
ln -sf "/mnt/lodev_default/n_prefix/${HOSTNAME}" "${N_PREFIX}"
