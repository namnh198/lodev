...
# When you run a command through it, stderr gets saved to /tmp/lodev-log-stderr-*.txt
# but only if the command fails.
#
# There are two different contexts where this is used:
#
# 1. During image build (additional layers in `.lodev/.webimageBuild/Dockerfile`)
#    - Use: log-stderr.sh <command>
#    - Errors saved here will trigger a rebuild without cache on next `lodev start`
...
#
# During `lodev start`, we call "log-stderr.sh --show" from app.Start() to display
...
# show stderr output
if [ "${show}" = "true" ]; then
  # find /tmp -maxdepth 1 -name 'lodev-log-stderr-*.txt': Searches for files matching the pattern.
  ...
  find /tmp -maxdepth 1 -name 'lodev-log-stderr-*.txt' -printf "%C@ %p\n" | sort -n | awk '{print $2}' | xargs --no-run-if-empty cat --squeeze-blank
  exit_code=$?
  # Logs are used to check if we want to run `docker-compose build` without cache in `lodev start`.
  ...
  rm -f /tmp/lodev-log-stderr-timeout-*.txt || true
  exit "${exit_code}"
fi
...
error_file="/tmp/lodev-log-stderr-${identifier}.txt"

# Timeout is used only in /start.sh, we want to log to a different file
if [ "${timeout}" = "true" ]; then
  error_file="/tmp/lodev-log-stderr-timeout-${identifier}-$(date +%s).txt"
fi

tmp_error_file="/tmp/tmp-lodev-log-stderr-${identifier}-$(date +%s).txt"
...
if [ "${timeout}" -gt 0 ] && [ "${exit_code}" -eq 124 ]; then
  ...
  echo "  - lodev exec ${command[*]}" | tee -a "${tmp_error_file}" >&2
  echo | tee -a "${tmp_error_file}" >&2
fi
