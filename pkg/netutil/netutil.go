...
func IsPortActive(port string) bool {
	...
	if err != nil {
		util.WarningMessage(fmt.Sprintf("Failed to get Docker IP address: %v", err))
		return false
	}
	...
	util.WarningMessage(fmt.Sprintf("Unable to properly check port status for %s:%s: err=%v", dockerIP, port, err))
	return false
}
