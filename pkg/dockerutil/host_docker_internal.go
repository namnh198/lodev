...
// reloading ~/.lodev/global_config.yaml).
...
func GetHostDockerInternal() *HostDockerInternal {
	hostDockerInternalOnce.Do(func() {
		...
		case util.IsWSL2MirroredMode() && !IsDockerDesktop():
...
func getWSL2PhysicalIP() (string, error) {
	out, err := lodevexec.RunHostCommand("ip", "-4", "route", "get", "1.1.1.1")
	if err != nil {
...
func getWSL2WindowsHostIP() (string, error) {
	...
	out, err := lodevexec.RunHostCommand("ip", "-4", "route", "show", "default")
